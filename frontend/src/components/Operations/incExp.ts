...
export class Operations {
    ...
    private initPeriodButtons(period: string): void {
        ...
        const applyButton: HTMLElement | null = document.getElementById('applyPeriod');
        if (applyButton) {
            applyButton.addEventListener('click', () => {
                const fromDateInput = document.getElementById('dateStartInput') as HTMLInputElement | null;
                const toDateInput = document.getElementById('dateEndInput') as HTMLInputElement | null;
    ...
    private async listOperations(period: string) {
        ...
        const result: ReturnObject<Object[]> = await OperationsService.getOperations(period);
        if (result.error) {
            alert('Выход из кабинета. Пожалуйста перезайдите.');
            return result.redirect ? this.openNewRoute(result.redirect) : null;
        }

        this.showOperations(result.response as OperationsType[]);
    };

    private showOperations(operations: OperationsType[]): void {
        operations.sort((a: { id: number }, b: { id: number }) => a.id - b.id);

        const recordsElement: HTMLElement | null = document.getElementById('operationsRecords');
        for (let i = 0; i < operations.length; i++) {
            const trElement = document.createElement('tr');

            const idCell = trElement.insertCell();
            idCell.innerText = operations[i].id.toString();
